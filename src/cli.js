// CLI implementation moved under src; adjusted imports relative to new location.
...
async function main(argv) {
  const uiAdapter = {
    ...
    async show(msg, data) {
      if (data !== undefined) {
        console.log(msg + ':');
        try { console.log(typeof data === 'string' ? data : JSON.stringify(data, null, 2)); } catch (_) { console.log(data); }
    ...
    async getInput(schemaProps, defaults) {
      const input = {};
      ...
      for (const key of keys) {
        ...
        if (typeof askDefault === 'object' && askDefault !== null && !Array.isArray(askDefault)) {
          ...
          answer = answer.split(',').map(x => x.trim()).filter(Boolean);
...
// Execute when run directly: `node src/cli.js`
...
if (isDirect) {
  ...
  main(argv).catch(err => { console.error(err?.stack || err); process.exitCode = 1; });
}